  ...
  <div class="stock-page">
      ...
      <el-col :xs="24" :sm="24" :md="16" :lg="16">
        <el-card shadow="hover" class="left-card">
          <div class="action-bar">
            ...
            >
            <div class="btn-group">
              <el-button type="success" icon="Plus" @click="inShow = true"
                >入库</el-button
              >
              <el-button type="warning" icon="Minus" @click="outShow = true"
                >出库</el-button
              >
            </div>
          </div>
          ...
          >
            ...
            >
              <template #default="{ row }">
                <span
                  :class="
                    row.currentQuantity <= row.warningLevel
                      ? 'warn-text'
                      : 'normal-text'
                  "
                >
        ...
        <el-card shadow="hover" class="record-card" style="margin-top: 20px">
          ...
          <div style="margin-bottom: 20px">
            ...
            <div
              v-if="filteredInRecords.length === 0"
              style="text-align: center; color: #909399; padding: 10px"
            >
              暂无入库记录
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              <el-table-column prop="receiver" label="领用人" min-width="80">
                <template #default="{ row }">
                  <el-link
                    type="primary"
                    @click="filterByReceiver(row.receiver)"
                  >
                    {{ row.receiver }}
                  </el-link>
                </template>
              ...
              <el-table-column prop="department" label="部门" min-width="80">
                <template #default="{ row }">
                  <el-link
                    type="primary"
                    @click="filterByDepartment(row.department)"
                  >
                    {{ row.department }}
                  </el-link>
                </template>
            ...
            <div
              v-if="filteredOutRecords.length === 0"
              style="text-align: center; color: #909399; padding: 10px"
            >
              暂无出库记录
            </div>
        ...
        >
          ...
          >
            ...
            <el-table-column label="操作" width="80">
              <template #default="{ row }">
                <el-button size="small" type="primary" @click="quickIn(row)"
                  >入库</el-button
                >
              </template>
            </el-table-column>
        ...
        <el-card shadow="hover" class="menu-stock-card">
          ...
          <el-table :data="menuList" size="small" border max-height="600">
            ...
            <el-table-column prop="stockQuantity" label="当前库存" width="80">
              <template #default="{ row }">
                <span :class="row.stockQuantity <= 0 ? 'warn-text' : ''">{{
                  row.stockQuantity
                }}</span>
              </template>
            </el-table-column>
    ...
    <el-dialog v-model="inShow" title="物品入库" width="550px">
      <div class="dialog-form">
        <div style="display: flex; gap: 8px; align-items: center">
          ...
          <el-button type="primary" plain @click="showNewItemDialog = true"
            >+ 新增物品</el-button
          >
        </div>
        <el-input-number
          v-model="inData.quantity"
          :min="1"
          placeholder="入库数量"
          style="width: 100%"
        />
    ...
    <el-dialog v-model="showNewItemDialog" title="新增物品" width="500px">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="createAndSelect"
          >保存并选中</el-button
        >
      </template>
    </el-dialog>
...
const outRules = {
  ...
  department: [
    { required: true, message: "领用部门自动填充", trigger: "change" },
  ],
...
const filteredOutRecords = computed(() => {
  ...
  if (filterReceiver.value && filterReceiver.value.trim()) {
    ...
    result = result.filter(
      (r) => r.receiver && r.receiver.toLowerCase().includes(keyword),
    );
  }
  // 按部门筛选
  if (filterDepartment.value && filterDepartment.value.trim()) {
    ...
    result = result.filter(
      (r) => r.department && r.department.toLowerCase().includes(keyword),
    );
  }
  return result;
});
...
const load = async () => {
  try {
    const res = await request.get("/warehouse/stock", {
      params: { itemName: name.value },
    });
    list.value = res.data || [];
  } catch (err) {
...
const doOut = async () => {
  ...
  if (stock.currentQuantity < outData.value.quantity) {
    return ElMessage.warning(
      `库存不足！当前库存：${stock.currentQuantity} ${stock.unit}`,
    );
  }
