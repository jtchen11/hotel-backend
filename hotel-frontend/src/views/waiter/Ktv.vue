﻿<template>
  <div class="ktv-container">
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          ...
          <el-button type="primary" icon="Refresh" @click="fetchRooms">刷新数据</el-button>
        </div>
      </template>

      <el-row :gutter="24">
        <el-col :span="8" v-for="room in roomList" :key="room.ktvId" class="mb-5">
          <el-card shadow="hover" class="room-card">
            <div class="room-top">
              <div class="room-name">{{ room.roomName }}</div>
              <el-tag :type="getStatusTagType(room.status)" size="small" effect="light">
                {{ room.status }}
              </el-tag>
            </div>
            ...
            <div class="room-middle">
              ...
              <div class="room-price-text">单价 <span class="price-num">¥{{ room.hourlyRate }}</span>/小时</div>
            ...
            <div class="room-bottom">
              <div class="btn-group">
                <el-button type="success" size="small" :disabled="room.status !== '空闲'" @click="openStartDialog(room)">
                  开房
                </el-button>
                <el-button type="danger" size="small" :disabled="room.status !== '使用中'" @click="openEndDialog(room)">
                  结账
                </el-button>
              </div>
              <el-select v-model="room.status" size="small" style="width:100px;margin-top:8px"
                         :disabled="room.status === '使用中'" @change="changeStatus(room)">
    ...
    <!-- 开房弹窗 -->
    <el-dialog title="新开KTV包房" v-model="startDialogVisible" width="35%" draggable>
      <el-form :model="startForm" label-width="90px">
        ...
        <el-form-item label="在住客人">
          <el-select v-model="startForm.guestId" filterable remote reserve-keyword
                     placeholder="输入姓名/手机号快速查询在住客人"
                     :remote-method="searchGuest"
                     :loading="guestLoading" style="width:100%">
            <el-option
    ...
    <!-- KTV结账弹窗 -->
    <el-dialog title="KTV账单确认结账" v-model="endDialogVisible" width="420px" draggable>
      <el-form :model="endForm" label-width="100px">
        ...
        <el-form-item label="使用时长(小时)">
          ...
          <div style="font-size:12px;color:#909399;margin-top:5px;">
            ...
            <span v-if="endForm.maxHour" style="color:#E6A23C;margin-left:8px;">
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmEndKtv">确认结账扣费</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const openStartDialog = (room) => {
  startForm.value = { ktvId: room.ktvId, roomName: room.roomName, guestId: null }
  startDialogVisible.value = true
  guestOptions.value = []
}

const searchGuest = async (keyword) => {
  if (!keyword) { guestOptions.value = []; return }
...
const onDurationBlur = () => {
  ...
  if (rounded > 0 && endForm.value.hourlyRate > 0) {
    endForm.value.totalFee = Math.round(rounded * endForm.value.hourlyRate * 100) / 100
  }
}
