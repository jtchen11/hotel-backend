  ...
  <div class="menu-container">
    <div class="header-card">
      ...
      <div class="guest-select">
        <el-select v-model="selectedGuestId" filterable remote reserve-keyword placeholder="选择在住客人"
                   :remote-method="searchGuest" :loading="guestLoading" style="width: 240px;height:38px">
          <el-option v-for="g in guestOptions" :key="g.guestId"
                     :label="`${g.name} (${g.phone}) - ${g.roomNumber || '无房间'}`" :value="g.guestId" />
        </el-select>
      </div>
      <div class="filter-box">
        <el-select v-model="category" @change="getMenu" placeholder="菜品分类" style="width:180px">
    ...
    <div class="main-content">
      <div class="menu-grid-wrapper">
        <div class="menu-grid">
          <div class="food-card" v-for="item in list" :key="item.menuId" :class="{ 'sold-out': item.status === '售罄' }">
            <div class="img-box">
              <img :src="item.image" alt="" @error="$event.target.src='https://img0.baidu.com/it/u=2538667333,3829791212&fm=253&fmt=auto&app=138&f=JPEG'">
            </div>
            <div class="info">
            ...
            <div class="ctrl">
              <el-button size="small" :disabled="item.status === '售罄'" @click="item.count = item.count > 0 ? item.count - 1 : 0">-</el-button>
              <span class="num-text">{{ item.count }}</span>
              <el-button size="small" :disabled="item.status === '售罄'" @click="item.count < 20 && item.count++">+</el-button>
              <el-button type="primary" size="small" class="add-btn" :disabled="item.status === '售罄'" @click="addCart(item)">点菜</el-button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="cart-sidebar">
        <div class="cart-header">
          <span>已点菜单</span>
          <el-badge :value="totalItemCount" :hidden="totalItemCount===0" class="item" />
        </div>
        <div class="cart-items" v-if="cart.length > 0">
          <div class="cart-item" v-for="(item, idx) in cart" :key="idx">
            ...
            <div class="item-ctrl">
              <el-button size="mini" @click="updateQuantity(item, -1)">-</el-button>
              <span>{{ item.count }}</span>
              <el-button size="mini" @click="updateQuantity(item, 1)">+</el-button>
              <el-button size="mini" type="danger" @click="removeItem(idx)">删除</el-button>
            </div>
            <div class="item-subtotal">小计：¥{{ (cleanPrice(item.price) * item.count).toFixed(2) }} 元</div>
          </div>
        </div>
...
const totalPrice = computed(() => {
  return cart.value.reduce((sum, item) => sum + (cleanPrice(item.price) * item.count), 0).toFixed(2)
})
...
.img-box { width:100%; height:160px; }
.img-box img { width:100%; height:100%; object-fit:cover; }
.info { padding:12px; }
.name { font-size:15px; font-weight:500; }
.spec { font-size:12px; color:#999; margin:3px 0; }
.desc { font-size:12px; color:#666; margin:3px 0; }
.price { color:#f56c6c; font-weight:bold; margin-top:6px; font-size:15px; }
.ctrl {
  display: flex;
  align-items: center;
  justify-content: space-between;
