<template>
  <div class="finance-dashboard">
    ...
    <el-row :gutter="20">
      <el-col :xs="24" :sm="24" :md="16" :lg="16">
        ...
        <el-card class="living-guests-card" style="margin-bottom: 20px">
          ...
          <el-table :data="livingGuests" border stripe v-loading="loadingLiving" style="width:100%">
            ...
            <el-table-column prop="unsettledAmount" label="挂账总额" width="120">
              <template #default="{row}">¥{{ row.unsettledAmount }}</template>
            </el-table-column>
            <el-table-column label="操作" width="195" fixed="right">
              <template #default="{row}">
                <el-button size="small" type="success" @click="addDepositQuick(row.guestId)">加收押金</el-button>
                <el-button size="small" type="danger" @click="goToSettle(row.guestId)">结账</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-card>
        ...
        <el-card class="attendance-card">
          ...
          <el-table :data="todayAttendance" border stripe size="small">
            ...
            <el-table-column prop="workHours" label="工作时长" width="100">
              <template #default="{row}">{{ row.workHours ? row.workHours + ' 小时' : '--' }}</template>
            </el-table-column>
          ...
          <div style="margin-top:12px; text-align:right">
            <el-button type="text" @click="$router.push('/finance/attendance')">查看全部</el-button>
          </div>
        </el-card>
      </el-col>
        ...
        <el-card class="pending-card" style="margin-bottom: 20px">
          ...
          <el-collapse-transition>
            <div v-show="showWarningList" class="warning-detail">
              <el-table :data="warningList" border size="small" v-loading="warningLoading">
    ...
    <el-dialog v-model="remarkDialogVisible" title="编辑备注" width="400px">
      <el-input v-model="remarkContent" type="textarea" :rows="3" placeholder="请输入备注信息" />
      <template #footer>
...
const depositForm = ref({ guestId: null, amount: 0, payMethod: '微信', remark: '' })
  ...
  try {
    ...
    if (chart) {
      chart.setOption({
        ...
        series: [{ type: 'line', data: data.map(d => d.revenue), smooth: true }]
      })
    }
  } catch (error) {
...
<style scoped>
.finance-dashboard { padding: 20px; background: #f5f7fa; min-height: 100vh; }
.stats-row { margin-bottom: 20px; }
.stats-card { text-align: center; border-radius: 12px; position: relative; }
.stats-number { font-size: 28px; font-weight: bold; color: #303133; }
.stats-label { font-size: 14px; color: #909399; margin-top: 8px; }
.help-icon { position: absolute; top: 12px; right: 12px; color: #909399; cursor: pointer; }
.living-guests-card, .attendance-card, .pending-card, .stats-chart-card { border-radius: 12px; margin-bottom: 20px; }
.pending-item {
  display: flex;
...
.pending-item:hover { background-color: #f5f7fa; }
.badge-right { margin-right: 12px; }
.remark-cell {
  cursor: pointer;
  display: flex;
  align-items: center;
