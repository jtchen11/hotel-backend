<template>
  <div class="salary-page">
    ...
    <el-card class="toolbar-card">
      <div class="toolbar">
        <el-date-picker
            ...
            style="width: 160px;"
        />
        <el-button type="primary" @click="generateSalary" :loading="generateLoading">生成工资</el-button>
        <el-button type="success" @click="exportExcel" :disabled="!salaryList.length">导出Excel</el-button>
        <el-button type="warning" @click="batchPay" :disabled="!selectedRows.length" :loading="batchPayLoading">批量发放</el-button>
      </div>
    </el-card>
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="24" :md="17" :lg="17">
        <el-card class="salary-list-card">
          ...
          >
            ...
            <!-- 姓名 -->
            <el-table-column prop="empName" label="姓名" fixed="left" width="80" />
            <!-- 基本工资 - 宽度 90 -->
            <el-table-column prop="baseSalary" label="基本工资" width="90" align="right">
              <template #default="{row}">¥{{ row.baseSalary }}</template>
            </el-table-column>
            <!-- 考勤扣款 - 宽度 90 -->
            <el-table-column prop="attendanceDeduction" label="考勤扣款" width="90" align="right">
              <template #default="{row}">¥{{ row.attendanceDeduction }}</template>
            </el-table-column>
            <el-table-column label="绩效奖金" width="100" align="right">
              <template #default="{row}">
                <div class="editable-cell" @click="openEditDialog(row, 'bonus')">
                  ¥{{ row.bonus }}
                </div>
              </template>
            </el-table-column>
            <el-table-column label="加班费" width="90" align="right">
              <template #default="{row}">
                <div class="editable-cell" @click="openEditDialog(row, 'overtimePay')">
                  ¥{{ row.overtimePay }}
                </div>
              </template>
            </el-table-column>
            <el-table-column label="其他加项" width="90" align="right">
              <template #default="{row}">
                <div class="editable-cell" @click="openEditDialog(row, 'otherAdd')">
                  ¥{{ row.otherAdd }}
                </div>
              </template>
            </el-table-column>
            <el-table-column label="其他扣款" width="90" align="right">
              <template #default="{row}">
                <div class="editable-cell" @click="openEditDialog(row, 'otherDeduct')">
                  ¥{{ row.otherDeduct }}
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="totalSalary" label="实发工资" width="100" align="right">
              <template #default="{row}">
            ...
            <el-table-column label="发放状态" width="80" align="center">
              <template #default="{row}">
                <el-tag :type="row.payStatus === '已发放' ? 'success' : 'danger'" size="small">
      ...
      <el-col :xs="24" :sm="24" :md="7" :lg="7">
        <el-card class="history-card">
          ...
          <el-table :data="historyList" border stripe v-loading="historyLoading" size="small" max-height="400">
            <el-table-column prop="yearMonth" label="月份" width="95" />
            <el-table-column prop="totalSalary" label="实发" width="90" align="right">
            ...
            <el-table-column label="操作" width="75" align="center">
              <template #default="{row}">
                <el-button type="text" size="small" @click="printSalarySlip(row)">工资条</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <div class="deduction-info">
            ...
            <p style="font-size:12px; color:#909399; margin-top:6px;">* 绩效奖金、加班费等为手动编辑项，不自动扣款</p>
          </div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="editDialogVisible" :title="'编辑 ' + editFieldName" width="400px">
...
const generateSalary = async () => {
  await ElMessageBox.confirm(`确认生成 ${currentMonth.value} 月份的工资吗？如果已存在将覆盖原有记录。`, '提示', { type: 'warning' })
  generateLoading.value = true
  try {
    await request.post('/hr/salary/generate', null, { params: { yearMonth: currentMonth.value } })
...
const batchPay = async () => {
  ...
  await ElMessageBox.confirm(`确认发放 ${ids.length} 笔工资吗？`, '提示', { type: 'warning' })
