<template>
  <div class="guest-bill-container">
        ...
        <el-card v-if="billData" class="bill-detail-card" style="height: 100%;">
          <template #header>
            <div class="bill-header">
              ...
              <div>
                <el-button type="success" size="small" @click="openDepositDialog('追加')">追加押金</el-button>
                <el-button type="warning" size="small" @click="openDepositDialog('退还')">退还押金</el-button>
                <el-button type="primary" size="small" @click="openAddMiscDialog">添加杂项</el-button>
              </div>
            </div>
          </template>
          ...
          <el-descriptions :column="3" border>
            <el-descriptions-item label="客人姓名">{{ billData.guestName }}</el-descriptions-item>
            <el-descriptions-item label="房间号">{{ billData.roomNumber }}</el-descriptions-item>
            <el-descriptions-item label="单日房费">¥{{ billData.roomPrice }}</el-descriptions-item>
            <el-descriptions-item label="入住时间">{{ formatDate(billData.checkInDate) }}</el-descriptions-item>
            <el-descriptions-item label="预离时间">{{ formatDate(billData.preLeaveDate )}}</el-descriptions-item>
            <el-descriptions-item label="押金余额">
    <span :class="billData.depositBalance >= 0 ? 'text-success' : 'text-danger'">
      ¥{{ billData.depositBalance }}
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <div class="info-card">餐饮挂账：¥{{ billData.unsettledFoodFee }}</div>
            </el-col>
            <el-col :span="8">
              <div class="info-card">杂项：¥{{ billData.unsettledDamageFee }}</div>
            </el-col>
            <el-col :span="8">
              <div class="info-card" style="background: #e6f7ff; border: 1px solid #91d5ff;">
          ...
          <span>
            <span style="font-weight: bold;">订单 #{{ group.orderId || '临时' }}</span>
            <span style="margin-left: 15px; color: #909399; font-size: 13px;">
              {{ group.createTime ? formatDateTime(group.createTime) : '' }}
            </span>
            <span style="margin-left: 15px; color: #909399; font-size: 13px;">
              操作员：{{ group.operator || '' }}
            </span>
          </span>
                    <span style="color: #f56c6c; font-weight: bold;">合计：¥{{ group.totalAmount.toFixed(2) }}</span>
                  </div>
                </template>
            ...
            <div v-if="groupedFoodDetails.length === 0" style="text-align: center; color: #909399; padding: 20px 0;">
            </div>
          </div>
          ...
          <div style="margin-top: 16px;">
            ...
            <div v-if="miscDetails.length === 0" style="text-align: center; color: #909399; padding: 20px 0;">
            </div>
          </div>
          ...
          <div class="settle-actions">
            <div class="discount-box">
              <span>折扣：</span>
              <el-input-number v-model="discountRate" :min="0.5" :max="1" :step="0.05" :precision="2" style="width: 120px" />
              <span class="discount-hint">（1表示无折扣）</span>
            </div>
            <el-button type="danger" size="large" @click="confirmSettle">确认结账</el-button>
          </div>
        </el-card>
    ...
    <el-row style="margin-top: 20px;">
      <el-col :span="24">
        <el-card class="settled-orders-card">
          ...
          <el-table :data="settledOrders" border stripe v-loading="loadingOrders">
            ...
            <el-table-column label="实收/退" width="120">
              <template #default="{row}">
                <span :class="(row.totalAmount - row.depositTotal) >= 0 ? 'text-danger' : 'text-success'">
                  {{ (row.totalAmount - row.depositTotal) >= 0 ? '补款: ' : '退款: ' }}
            ...
            <el-table-column label="明细" width="320">
              <template #default="{row}">
                <div class="detail-summary">
                  ...
                  <el-button type="text" style="margin-left: auto;" @click="showOrderDetails(row.orderId)">详情</el-button>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="100">
              <template #default="{row}">
                <el-button type="text" @click="printBill(row.orderId)">打印</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
    ...
    <!-- 押金操作弹窗（不变） -->
    <el-dialog v-model="depositDialogVisible" :title="depositType === '追加' ? '追加押金' : '退还押金'" width="400px">
      <el-form :model="depositForm" label-width="80px">
        <el-form-item label="金额">
          <el-input-number v-model="depositForm.amount" :precision="2" :step="100" :min="0" />
        </el-form-item>
    ...
    <el-dialog v-model="orderDetailsDialogVisible" title="订单明细" width="600px">
    ...
    <el-dialog v-model="miscDialogVisible" title="添加杂项费用" width="400px">
      <el-form :model="miscForm" label-width="80px">
        <el-form-item label="项目名称">
          <el-input v-model="miscForm.itemName" placeholder="如：洗衣费、加床费等" />
        </el-form-item>
        <el-form-item label="金额">
          <el-input-number v-model="miscForm.amount" :precision="2" :step="10" :min="0" />
        </el-form-item>
        <el-form-item label="备注">
...
const groupedFoodDetails = computed(() => {
  const foodItems = billData.value?.unsettledDetails?.filter(d => d.itemType === '餐饮') || []
...
const miscDetails = computed(() => {
  return billData.value?.unsettledDetails?.filter(d => d.itemType === '杂项') || []
})
// 显示订单明细
...
const filteredGuests = computed(() => {
  ...
  return allGuests.value.filter(g =>
      g.name.toLowerCase().includes(kw) || g.roomNumber.toLowerCase().includes(kw)
  )
})
...
const confirmSettle = async () => {
  ...
  await ElMessageBox.confirm('确认结账？结账后客人将离店', '提示', { type: 'warning' })
  const res = await request.post('/finance/settle', null, {
    params: { guestId: selectedGuestId.value, discountRate: discountRate.value }
  })
  ...
  // 询问是否打印账单
  if (res.data.orderId) {
    ElMessageBox.confirm('是否打印账单？', '提示', { confirmButtonText: '打印', cancelButtonText: '取消', type: 'info' })
        .then(() => {
...
.guest-list-card, .bill-detail-card {
  border-radius: 12px;
}
.guest-list-card {
  height: 100%;
