  ...
  <div class="employee-container">
      ...
      <el-col :xs="24" :sm="24" :md="8" :lg="8">
        <el-card class="employee-list-card">
          ...
          <div style="display: flex; gap: 8px; margin-bottom: 12px">
            <el-select v-model="filterRole" clearable placeholder="角色" size="small" @change="loadEmployeeList">
            ...
            <el-select v-model="filterStatus" clearable placeholder="状态" size="small" @change="loadEmployeeList">
              <el-option label="在职" value="在职" />
              <el-option label="离职" value="离职" />
            </el-select>
          </div>
          <div class="employee-list" style="max-height: calc(100vh - 280px); overflow-y: auto">
            <div
            ...
            >
              <div class="emp-info">
                ...
                <span class="emp-role-dept">{{ emp.role }} - {{ emp.department || '无部门' }}</span>
              </div>
              <div class="emp-status">
                <el-tag :type="emp.status === '在职' ? 'success' : 'info'" size="small">
                  {{ emp.status }}
                </el-tag>
              </div>
            </div>
      ...
      <el-col :xs="24" :sm="24" :md="16" :lg="16">
        <el-card v-if="currentEmployee" class="employee-detail-card">
          <template #header>
            <div class="detail-header">
              ...
              <div>
                ...
                >重新聘用</el-button>
                <el-button type="danger" size="small" @click="handleDelete(currentEmployee.empId)">删除</el-button>
              </div>
            </div>
          </template>
          ...
          <el-descriptions :column="3" border>
            ...
            <el-descriptions-item label="性别">{{ currentEmployee.gender || '未知' }}</el-descriptions-item>
            <el-descriptions-item label="年龄">{{ currentEmployee.age || '-' }}</el-descriptions-item>
            <el-descriptions-item label="身份证">{{ currentEmployee.idCard }}</el-descriptions-item>
            ...
            <el-descriptions-item label="基本工资">¥{{ currentEmployee.baseSalary }}</el-descriptions-item>
            <el-descriptions-item label="角色">{{ currentEmployee.role }}</el-descriptions-item>
            <el-descriptions-item label="状态">{{ currentEmployee.status }}</el-descriptions-item>
            <el-descriptions-item label="入职日期">{{ currentEmployee.hireDate || '-' }}</el-descriptions-item>
            <el-descriptions-item label="离职日期">{{ currentEmployee.leaveDate || '-' }}</el-descriptions-item>
          </el-descriptions>

          <!-- 工资历史 -->
          <el-divider>工资历史</el-divider>
          <el-table :data="salaryHistory" border size="small" v-loading="loadingSalary">
            ...
            <el-table-column prop="payStatus" label="发放状态" width="100">
              <template #default="{row}">
                <el-tag :type="row.payStatus === '已发放' ? 'success' : 'danger'">{{ row.payStatus }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="100">
              <template #default="{row}">
                <el-button type="text" @click="printSalarySlip(row)">工资条</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <el-collapse v-model="activeAttendanceMonth" v-loading="loadingAttendance">
            <el-collapse-item
            ...
            >
              <template #title>
                <div style="display: flex; justify-content: space-between; width: 100%; padding-right: 20px;">
                  ...
                  <span>
          出勤：{{ monthData.presentDays }} 天 &nbsp;|&nbsp;
          迟到：{{ monthData.lateCount }} 次 &nbsp;|&nbsp;
          早退：{{ monthData.earlyLeaveCount }} 次 &nbsp;|&nbsp;
          缺勤/旷工：{{ monthData.absentTotal }} 天
      <span v-if="monthData.missingDays > 0" style="color: #909399; font-size: 12px;">
      ...
      请假：{{ monthData.leaveCount }} 天
              ...
              <el-table :data="monthData.details" border size="small">
                ...
                <el-table-column prop="status" label="状态" width="100">
                  <template #default="{row}">
                    <el-tag :type="statusTagType(row.status)" size="small">{{ row.status || '未打卡' }}</el-tag>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="workHours" label="工作时长" width="100">
                  <template #default="{row}">{{ row.workHours !== null ? row.workHours : '-' }}</template>
                </el-table-column>
              </el-table>
            </el-collapse-item>
          </el-collapse>
          <el-empty v-if="!attendanceMonths.length && !loadingAttendance" description="暂无考勤记录" />
        </el-card>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑员工" width="500px">
      <el-form :model="editForm" :rules="editRules" ref="editFormRef" label-width="80px">
        <el-form-item label="姓名"><el-input v-model="editForm.empName" /></el-form-item>
        ...
        <el-form-item label="身份证"><el-input v-model="editForm.idCard" :disabled="true" /></el-form-item>
        ...
        <el-form-item label="密码">
          <el-input v-model="addForm.password" placeholder="不填默认为123456" show-password />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="addDialogVisible" title="新增员工" width="500px">
      <el-form :model="addForm" :rules="addRules" ref="addFormRef" label-width="80px">
        <el-form-item label="姓名"><el-input v-model="addForm.empName" /></el-form-item>
        <el-form-item label="性别">
        ...
        <el-form-item label="身份证"><el-input v-model="addForm.idCard" /></el-form-item>
        <el-form-item label="电话"><el-input v-model="addForm.phone" /></el-form-item>
        <el-form-item label="学历"><el-input v-model="addForm.education" /></el-form-item>
        <el-form-item label="部门"><el-input v-model="addForm.department" /></el-form-item>
        <el-form-item label="职务"><el-input v-model="addForm.position" /></el-form-item>
        <el-form-item label="基本工资">
...
import { listEmployees, addEmployee, updateEmployee, deleteEmployee } from '@/api/finance'
...
const editForm = reactive({ empId: null, empName: '', gender: '男', idCard: '', phone: '', education: '', department: '', position: '', baseSalary: 0, role: '' })
const addDialogVisible = ref(false)
const addForm = reactive({ empName: '', gender: '男',   age: null, idCard: '', phone: '', education: '', department: '', position: '', baseSalary: 0, role: '',hireDate: '', password: ''   })
...
const loadAttendanceRecords = async (empId) => {
  ...
  try {
    ...
    records.forEach(att => {
      ...
      if (att.status === '正常' || att.status === '迟到' || att.status === '早退') {
...
const selectEmployee = async (empId) => {
  ...
  try {
    const salaryRes = await request.get('/hr/salary/list', { params: { empId, page: 1, size: 100 } })
...
const handleResign = async (empId) => {
  await ElMessageBox.confirm('确定将该员工设为离职状态吗？', '提示', { type: 'warning' })
...
const handleReinstate = async (empId) => {
  await ElMessageBox.confirm('确定重新聘用该员工吗？', '提示', { type: 'info' })
...
<style scoped>
.employee-container { padding: 20px; background: #f5f7fa; min-height: 100vh; }
.employee-list-card { border-radius: 12px; height: calc(100vh - 70px); overflow-y: auto; }
.employee-item { padding: 12px; border-bottom: 1px solid #eef2f6; cursor: pointer; transition: background 0.2s; display: flex; justify-content: space-between; align-items: center; }
.employee-item:hover { background-color: #f5f7fa; }
.employee-item.active { background-color: #e6f7ff; border-left: 3px solid #409eff; }
.emp-info { display: flex; flex-direction: column; }
.emp-name { font-weight: 500; }
.emp-role-dept { font-size: 12px; color: #909399; }
.employee-detail-card { border-radius: 12px; }
