  ...
  <div class="attendance-page">
    ...
    <el-row :gutter="20" class="stats-row">
      <template v-if="!selectedEmpId">
        ...
        <el-col :span="6">
          <el-card shadow="hover" class="stats-card">
            <div class="stats-number">{{ selectedDateStats.presentCount }}/{{ selectedDateStats.totalEmployees }}</div>
            <div class="stats-label">出勤人数</div>
          </el-card>
        </el-col>
        <el-col :span="6">
          <el-card shadow="hover" class="stats-card">
            <div class="stats-number">{{ selectedDateStats.lateEarlyCount }}</div>
      ...
      <template v-else>
        ...
        <el-col :span="5">
          <el-card shadow="hover" class="stats-card">
            <div class="stats-number" style="color: #e6a23c;">{{ empStats.lateEarlyCount }}</div>
            <div class="stats-label">迟到/早退</div>
          </el-card>
        </el-col>
        <el-col :span="5">
          <el-card shadow="hover" class="stats-card">
            <div class="stats-number" style="color: #409eff;">{{ empStats.leaveDays }}</div>
            <div class="stats-label">请假</div>
          </el-card>
        </el-col>
        <el-col :span="4">
          <el-card shadow="hover" class="stats-card">
            <div class="stats-number" style="color: #f56c6c;">{{ empStats.absentDays }}</div>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center">
              ...
              <div>
                <el-date-picker
                    ...
                    style="width: 140px"
                />
                <el-button v-if="selectedEmpId" type="text" @click="clearSelectedEmp" style="margin-left: 12px">返回全局视图</el-button>
              </div>
            </div>
          </template>
            ...
            <template #date-cell="{ data }">
              <div class="calendar-cell" :class="getDateStatusClass(data.day)">
                ...
                <div class="status-dots">
                  <template v-if="!selectedEmpId">
                    <span v-if="getDateStatus(data.day).hasNormal" class="dot dot-normal"></span>
                    <span v-if="getDateStatus(data.day).hasLate" class="dot dot-late"></span>
                    <span v-if="getDateStatus(data.day).hasAbsent" class="dot dot-absent"></span>
                    <span v-if="getDateStatus(data.day).hasNotPunch" class="dot dot-not"></span>
                    <span v-if="getDateStatus(data.day).hasLeave" class="dot dot-leave"></span>
                  </template>
                  <template v-else>
      ...
      <el-col :xs="24" :sm="24" :md="14" :lg="14">
        ...
        <el-card class="punch-card">
          ...
          <div style="display: flex; flex-wrap: nowrap; align-items: center; gap: 8px;">
            <el-select v-model="punchEmpId" placeholder="员工" clearable filterable style="width: 100px;">
              <el-option v-for="e in activeEmployees" :key="e.empId" :label="e.empName" :value="e.empId" />
            </el-select>
            <el-date-picker
                v-model="punchDate"
                type="date"
                ...
                style="width: 150px;"
            />
            <el-button type="primary" size="default" @click="doPunch('in')">上班</el-button>
            <el-button type="warning" size="default" @click="doPunch('out')">下班</el-button>
            <el-button type="success" size="default" @click="doLeave">请假</el-button>
          </div>
        </el-card>
        ...
        <el-card class="day-attendance-card">
          <template #header>
            <div style="display: flex; justify-content: space-between; align-items: center">
              ...
              <el-button type="text" size="small" @click="refreshTable">刷新</el-button>
            </div>
          </template>
          <div :class="{ 'scroll-table': selectedEmpId }">
            <el-table :data="tableData" border stripe size="small" v-loading="tableLoading" max-height="500">
              <el-table-column prop="empName" label="员工姓名" v-if="!selectedEmpId" />
              ...
              <el-table-column label="操作" width="80" v-if="!selectedEmpId">
                <template #default="{row}">
                  <el-button type="text" @click="editAttendance(row)">编辑</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
    ...
    <el-dialog v-model="editDialogVisible" title="编辑考勤" width="500px">
      <el-form :model="editForm" label-width="100px">
        ...
        <el-form-item label="工作时长">
          <el-input-number v-model="editForm.workHours" :precision="1" :step="0.5" />
        </el-form-item>
...
const editForm = ref({ attendanceId: null, empId: null, workDate: '', status: '', workHours: 0, remark: '' })
// 统计数据
...
const loadGlobalData = async () => {
  ...
  for (const att of records) {
    ...
    if (!dateMap[date]) dateMap[date] = { hasNormal: false, hasLate: false, hasAbsent: false, hasLeave: false, hasNotPunch: true }
    const status = att.status
    if (status === '正常') { dateMap[date].hasNormal = true; dateMap[date].hasNotPunch = false }
    else if (status === '迟到' || status === '早退') { dateMap[date].hasLate = true; dateMap[date].hasNotPunch = false }
    else if (status === '旷工') { dateMap[date].hasAbsent = true; dateMap[date].hasNotPunch = false }
    else if (status === '请假') { dateMap[date].hasLeave = true; dateMap[date].hasNotPunch = false }
  }
...
const loadSelectedDateStats = async (dateObj) => {
  ...
  let presentCount = 0, lateEarlyCount = 0, leaveCount = 0
...
const getDateStatus = (date) => {
  return calendarDataMap.value[date] || { hasNormal: false, hasLate: false, hasAbsent: false, hasLeave: false, hasNotPunch: true }
}
...
const loadEmpData = async (empId) => {
  ...
  const records = res.data.records || []
  const map = {}
  let normalDays = 0, lateCount = 0, earlyLeaveCount = 0, leaveDays = 0, absentCount = 0
  ...
  // 员工模式表格显示整月记录
  tableData.value = records.sort((a,b) => a.workDate.localeCompare(b.workDate))
}
