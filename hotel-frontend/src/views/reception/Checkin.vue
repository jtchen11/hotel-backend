<template>
  <div class="checkin-page" style="padding: 20px; max-width: 1300px; margin: 0 auto;">
    ...
    <div style="display:flex;gap:12px;align-items:center;margin-bottom:20px;">
      ...
      />
      <el-button type="primary" @click="loadAllRoomByDate">应用日期筛选</el-button>
    </div>
    ...
    <div v-if="step === 1">
      <div v-loading="loading" class="room-grid">
        <div
            ...
            :key="r.roomId"
            class="room-card"
            :class="{ selected: selectedRoom?.roomId === r.roomId, disable: r.isBusy }"
            @click="pickRoom(r)"
        >
      ...
      <div style="text-align: right; margin-top: 20px;">
        <el-button type="primary" :disabled="!selectedRoom || selectedRoom.isBusy" @click="next">下一步</el-button>
      </div>
    </div>
    ...
    <div v-else-if="step === 3">
      ...
      <div style="display: flex; gap: 10px; justify-content: right; margin-top: 20px;">
        <el-button @click="prev">上一步</el-button>
        <el-button type="primary" @click="submitCheckIn">确认提交预订</el-button>
      </div>
    </div>
  </div>
</template>
...
const rules = {
  ...
  nativePlace: [{ required: true, message: '请选择或输入籍贯', trigger: 'change' }]
}
...
const loadAllRoomByDate = async () => {
  if (!searchIn.value || !searchOut.value) return ElMessage.warning('请选择完整的日期范围')
  loading.value = true
  try {
