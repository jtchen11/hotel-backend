<template>
  <div class="home-page" style="padding:20px;">
    <!-- 顶部统计卡片 -->
    <div class="top-card-box" style="display:grid;grid-template-columns: repeat(5,1fr);gap:20px;margin-bottom:35px;">
      <el-card shadow="hover">
        <div style="font-size:32px;font-weight:bold">{{ roomData.total }}</div>
        <div style="color:#666">总房间数</div>
      </el-card>
      <el-card shadow="hover">
        <div style="font-size:32px;font-weight:bold;color:#52c41a">{{ roomData.free }}</div>
        <div style="color:#666">空闲</div>
      </el-card>
      <el-card shadow="hover">
        <div style="font-size:32px;font-weight:bold;color:#fa8c16">{{ roomData.used }}</div>
        <div style="color:#666">占用</div>
      </el-card>
      <el-card shadow="hover">
        <div style="font-size:32px;font-weight:bold;color:#909399">{{ roomData.repair }}</div>
        <div style="color:#666">维修中</div>
      </el-card>
      <el-card shadow="hover">
        <div style="font-size:32px;font-weight:bold;color:#409EFF">{{ roomData.clean }}</div>
    ...
    <div style="display:flex; gap:20px;">
        ...
        <el-card class="section-card" style="margin-top:20px;">
          ...
          >
            ...
            <el-table-column prop="consume" label="挂账总额" width="120">
              <template #default="{row}">¥{{ row.consume.toFixed(2) }}</template>
            </el-table-column>
          </el-table>
        </el-card>
      </div>
      ...
      <div style="flex:1; display:flex; flex-direction:column; gap:20px;">
        ...
        <el-card class="msg-card">
          ...
          <div v-if="pendingBookings.length === 0" class="empty-msg">暂无待入住预订</div>
          <div v-else class="booking-list">
            <div v-for="b in pendingBookings" :key="b.guestId" class="booking-item">
        ...
        <el-card class="msg-card">
          <template #header>
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <span>
                💬 未读留言
                <el-badge :value="unreadCount" :hidden="unreadCount===0" type="danger" style="margin-left:8px;" />
              </span>
              <el-button type="text" @click="$router.push('/reception/messages')">查看全部</el-button>
            </div>
          </template>
          <div v-if="unreadMessages.length === 0" class="empty-msg">暂无未读留言</div>
          <div v-else class="msg-list">
            <div v-for="msg in unreadMessages" :key="msg.messageId" class="msg-item">
              <div class="msg-content">{{ msg.content }}</div>
              <div class="msg-meta">{{ msg.roomNumber }} · {{ msg.createTime }}</div>
            </div>
          </div>
        </el-card>
      </div>
    ...
    <el-dialog title="办理入住" v-model="checkinDialogVisible" width="400px">
      <el-form :model="checkinForm" label-width="100px">
        ...
        <el-form-item label="押金金额">
          <el-input-number v-model="checkinForm.depositAmount" :min="200" :precision="2" :step="100" />
        </el-form-item>
...
// 表格行样式
    rowStyle({ row }) {
      ...
      console.log('rowStyle:', row.roomNumber, row.preLeaveDate, 'isTodayLeave:', isToday);
      if (isToday) {
    ...
    statusTagType(status) {
      const map = { '空闲': 'success', '维修中': 'info', '打扫中': 'warning', '占用': 'danger' , '已预订': 'primary'}
    ...
    async changeStatus(row) {
      ...
      await request.put(`/reception/room/updateStatus?roomId=${row.roomId}&status=${row.status}`)
