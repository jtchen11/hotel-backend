<template>
  <div class="group-checkin-page" style="padding:20px;max-width:1300px;margin:0 auto;">
    ...
    <div v-if="step===1">
      <div v-loading="loading" class="room-grid">
        <div
            ...
            :key="item.roomId"
            class="room-card"
            :class="{ selected: selectRoomIds.includes(item.roomId), disable:item.isBusy }"
      ...
      <div style="text-align:right">
        <el-button type="primary" :disabled="selectRoomIds.length===0" @click="next">下一步</el-button>
      </div>
    </div>
    ...
    <div v-else-if="step===3">
      ...
      <div style="display:flex;gap:10px;justify-content:right;margin-top:20px;">
        <el-button @click="prev">上一步</el-button>
        <el-button type="primary" @click="submitGroupCheckIn">确认批量预订</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    async loadRoomByDate(){
      if(!this.searchIn||!this.searchOut) return this.$message.warning('请选择完整的日期范围')
      this.loading=true
    ...
    async submitGroupCheckIn(){
      try {
        ...
        if(res.code===200){
          ...
          this.step=1
          this.selectRoomIds=[]
          this.group={groupName:'',groupLeader:'',leaderPhone:'',personCount:''}
          this.loadRoomByDate()
        }else{
...
.tip {color:#ff6000;font-weight:bold;}
</style>