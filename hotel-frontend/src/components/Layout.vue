  ...
  <el-container style="height: 100vh">
    ...
    <el-container>
      <el-header style="background-color: #fff; border-bottom: 1px solid #e6e6e6; display: flex; justify-content: space-between; align-items: center">
        <span>欢迎，{{ userStore.userInfo.empName }} ({{ userStore.userInfo.role }})</span>
...
import { House, User, Edit, Ticket, PieChart, Goods, Service, Money, DataAnalysis } from '@element-plus/icons-vue';
...
const menus = computed(() => {
  ...
  if (role === '前台接待员') {
    ...
    return [
      { path: '/gm/statistics', title: '统计图表', icon: PieChart },
    ];
  }
  return [];
});
