...
const setConfig = (options = {}) => {
  ...
  if (!config.ak && !config.serviceHost) {
    console.error("\u8BF7\u914D\u7F6E ak\uFF08\u767E\u5EA6\u5730\u56FE\u5F00\u53D1\u8005\u5E73\u53F0\u7533\u8BF7\u7684\u5BC6\u94A5\uFF09\u6216 serviceHost\uFF08\u4EE3\u7406\u670D\u52A1\u5668\u5730\u5740\uFF09");
  }
  if (config.ak && config.serviceHost) {
    console.warn("\u540C\u65F6\u63D0\u4F9B\u4E86 ak \u548C serviceHost\uFF0C\u5C06\u4F18\u5148\u4F7F\u7528 serviceHost\uFF08\u4EE3\u7406\u6A21\u5F0F\uFF09");
  }
};
const getConfig = () => {
  return config;
...
const _sfc_main$x = {
  ...
  methods: {
    ...
    init(BMap2) {
      ...
      let loadNum = 0;
      this.$emit("init", { BMap: BMap2, map, mapKey });
      EvenBus.$emit("init", { BMap: BMap2, map, mapKey });
      map.addEventListener("tilesloaded", () => {
        if (!loadNum) {
          loadNum++;
          this.$emit("ready", { BMap: BMap2, map, mapKey });
          EvenBus.$emit("ready", { BMap: BMap2, map, mapKey });
        }
      });
      map.addEventListener("loaded", () => {
        this.$emit("loaded", { BMap: BMap2, map, mapKey });
        EvenBus.$emit("loaded", { BMap: BMap2, map, mapKey });
      });
    },
    ...
    getMapScript() {
      if (!window.BMap) {
        ...
        window.BMap._preloader = new Promise((resolve, reject) => {
          ...
          if (config2.serviceHost) {
            window._BMapSecurityConfig = {
              serviceHost: config2.serviceHost
            };
            window.document.body.appendChild($script);
            switch (config2.type) {
              case "WebGL":
                $script.src = `${config2.serviceHost}api?v=1.0&type=webgl&callback=_initBaiduMap`;
                break;
              default:
                $script.src = `${config2.serviceHost}api?v=${config2.v}&callback=_initBaiduMap`;
            }
          } else {
            window.document.body.appendChild($script);
            switch (config2.type) {
              case "WebGL":
                $script.src = `https://api.map.baidu.com/api?v=1.0&type=webgl&ak=${config2.ak}&callback=_initBaiduMap`;
                break;
              default:
                $script.src = `https://api.map.baidu.com/api?v=${config2.v}&ak=${config2.ak}&callback=_initBaiduMap`;
            }
          }
        });
  ...
  created() {
    ...
    this.serviceHost && (options.serviceHost = this.serviceHost);
...
let geocoder;
const useGeocoder = (options) => {
  return new Promise((resolve) => {
    if (geocoder) {
      resolve(geocoder);
    } else {
      useMap().then((BMap2) => {
        geocoder = new BMap2.Geocoder(options);
        resolve(geocoder);
      });
    }
  });
